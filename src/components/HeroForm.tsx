        ...
        {preview ? (
            ...
            {slides?.map((slide,index)=>{
               ...
               >
                 ...
                 <FormItemUpload id={slide.id} label="Background URL(1920x800):" name="background"  value={slide.background} onChange={(e)=>{
                     console.log('slide',slide)
                     onChange(e,slide.id,"background")
                 }} />
                 <FormItem label="Tag" name="tag" type="text" placeholder="Type tag" value={slide.tag} onChange={(e)=>{
                     onChange(e,slide.id,"tag")
                 }} />
                 <FormItem label="Title" name="title" type="text" placeholder="Type title" value={slide.title} onChange={(e)=>{
                     onChange(e,slide.id,"title")
                 }} />
                 <FormItem label="Description" name="description" type="textarea" placeholder="Type description" value={slide.description} onChange={(e)=>{
                     onChange(e,slide.id,"description")
                 }} />
                 <FormItem label="Button Link" name="btnLink" type="text" placeholder="Type button link" value={slide.btnLink} onChange={(e)=>{
                     onChange(e,slide.id,"btnLink")
                 }} />
                  <FormItem label="Button Text" name="btnText" type="text" placeholder="Type button text" value={slide.btnText} onChange={(e)=>{
                     onChange(e,slide.id,"btnText")
                 }} />
 
 
 
             <button type="button" className="mt-4 rounded bg-red-500 px-4 py-1 text-white" onClick={()=>handleRemoveSlide(slide.id)}>Remove Slide</button>
         ...
         <button type="button" className="mt-4 rounded bg-blue-500 px-4 py-2 text-white" onClick={handleAddSlide}>Add Slide</button>
