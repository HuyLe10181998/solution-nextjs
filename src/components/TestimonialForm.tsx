...
function TestimonialForm({testimonialData}:{testimonialData:TestimonialData}){

    const [users,setUsers] = useState<UserComment[]>(testimonialData?.users || [])
    ...
    const checkUser = ()=>{
        return users.every((user)=>{
            return typeof user.job === 'string' && typeof user.text === 'string' && typeof user.avatar === 'string'
        })
    }
        ...
        {preview ? (
            ...
            {users?.map((user,index)=>{
               ...
               >
                 ...
                 <FormItemUpload id={user.id} label="Avatar:" name="image"  value={user.avatar} onChange={(e)=>{
                     onChange(e,user.id,"image")
                 }} />
                 <FormItem label="Name" name="name" type="text" placeholder="Type name" value={user.name} onChange={(e)=>{
                     onChange(e,user.id,"name")
                 }} />
                 <FormItem label="Job" name="job" type="text" placeholder="Type job" value={user.job} onChange={(e)=>{
                     onChange(e,user.id,"job")
                 }} />
                 <FormItem label="Text" name="text" type="text" placeholder="Type text" value={user.text} onChange={(e)=>{
                     onChange(e,user.id,"text")
                 }} />
                 <FormItem label="Date" name="date" type="date" placeholder="Type date" value={new Date(user.date as string)} onChange={(e)=>{
             ...
             <button type="button" className="mt-4 rounded bg-red-500 px-4 py-1 text-white" onClick={()=>handleRemoveUser(user.id)}>Remove User</button>
         ...
         <button type="button" className="mt-4 rounded bg-blue-500 px-4 py-2 text-white" onClick={handleAddUser}>Add User</button>
