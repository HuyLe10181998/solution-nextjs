...
function NewsForm({news}:{news?: News }){
    ...
    async function handleNews(e: FormEvent<HTMLFormElement>) {
        ...
        try {
          if(isEdit){
            ...
            result = await updateNew(news?.id,data)
          }else{
    ...
    return   <form onSubmit={handleNews}
  ...
  >
    ...
    <FormItemUpload label="Thumb Image" name="thumb"  value={news?.thumb || ''} onChange={() => {}} />
    <FormItem label="Title" name="title" type="text" placeholder="Type title here" value={news?.title || ''} onChange={() => {}} />


    <FormItem label="Description" name="description" type="text" placeholder="Type description here" value={news?.description || ''} onChange={() => {}} /> 
