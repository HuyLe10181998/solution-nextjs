          ...
          <div className="py-4 overflow-y-auto">
            <ul className="space-y-2 font-medium">
              ...
              const isActive = item.link === pathname || item.children?.some((child) => child.link === pathname)
                ...
                return   (
                  <li key={item.link}>
                    <Link href={item.link} className={`flex items-center p-2 text-gray-900 hover:!text-white rounded-lg  dark:hover:bg-blue-600 group ${isActive ? "text-primary-default" : ""}`}>
                        ...
                        {item.children.map((child) => {
                          ...
                          return (
                            <li className="!list-disc" key={child.link}>
                              <Link href={child.link} className={`flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-100 hover:text-white dark:hover:bg-gray-700 group ${isActive ? "!text-primary-default" : ""}`}>
